...
GameManager::GameManager()
{
	...
	m_istagenumber = 1;
	char buf[256];
	sprintf(buf, "mode con: lines=%d cols=%d", m_iHeight + 4, m_iWith * 2);
	system(buf);
	m_bchect = false;
	m_binterface = false;
}
...
void GameManager::MainSrean()
{
	...
	while (gameover)
	{
		...
		in_face.DrawMidText("☆ ★ 베 네 치 아 ★  ☆", m_iWith, m_iHeight * 0.2f);
		in_face.DrawMidText("1. Game Start", m_iWith, m_iHeight * 0.4f);
		in_face.DrawMidText("2. Rank", m_iWith, m_iHeight * 0.4f + 3);
		in_face.DrawMidText("3. Exit", m_iWith, m_iHeight * 0.4f + 6);
		underinterface();
		switch (in_face.MenuSelectCursor(3, 3, m_iWith *0.4f, m_iHeight * 0.4f))
		{
		...
		case MAINSELECT_RANK:
			rank.Ranking();
			rank.Rankingsystem(m_iWith, m_iHeight);
			break;
