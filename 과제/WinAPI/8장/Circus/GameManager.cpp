...
void GameManager::Update()
{
	...
	else if (m_fDeltaTime >= 0.08f)
	{
		...
		{
			if (m_bcrash == true)//재충돌 지연시간(무적시간)
				m_iInvincibilityTime++;
			if (m_state == WIN )
			{
				m_iInvincibilityTime = 0;
			...
			if (m_rightring[1]->randommoneyring() == RANDOM) //랜덤숫자가 1일때
			{
