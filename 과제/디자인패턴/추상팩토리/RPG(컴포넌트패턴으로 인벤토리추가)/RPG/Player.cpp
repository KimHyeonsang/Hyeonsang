...
void Player::showplayer(int x, int y, Weapon* hm)
{
	map.gotoxy(x, y);
	...
	y++;
	x -= 10;
	cout << "공격력 :" << hm.weaponAttack() << "무기이름 : " << hm.WeaponName();
...
Weapon * shipCharactor::CreateWeapon()
{
	return new Dagger();
}

JobClass * shipCharactor::CreateJobClass()
{
	return new Wizard();
}