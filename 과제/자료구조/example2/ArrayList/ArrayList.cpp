...
void main()
{
	...
	for (int i = 1; i <= LIST_LEN; i++)
		Linsert(list, i);//1~9까지 리스트로 저장
	LFirst(list, &num);
	sum += num;
	while (cut)
	{
		if (LNext(list, &num) == true)
			sum += num;
		else
			cut = LNext(list, &num);
	}
	printf("조건 2. 총 합 : %d 입니다.\n", sum);
	printf("2의 배수와 3의 배수에 해당하는 값을 모두 삭제.\n");
	LFirst(list, &num);
	printf("삭제된 값 : ");
	...
	printf("\n");
	printf("3 .리스트에 저장된 데이터를 순서대로 출력.\n");
	for (int i = 0; i < LIST_LEN - count; i++)
		printf("%d ",list->arr[i]);
}
